...
{
	unsigned char tmp[16];
	// Di chuyển từng hàng của ma trận state sang phải với số lượng khác nhau
	/* Column 1 */
	tmp[0] = state[0];
	tmp[1] = state[13];
	tmp[2] = state[10];
...
/* Thay thế từng byte trong mảng state bằng giá trị tương ứng của bảng S-Box
 * S-Box là một bảng tra cứu được sử dụng trong AES để thay thế các byte
 * trong quá trình mã hóa và giải mã.
 * Bảng S-Box được thiết kế để cung cấp tính bảo mật cao hơn
 * bằng cách làm cho các byte đầu vào trở nên khó đoán hơn.
 */
